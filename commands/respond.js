...
module.exports = (bot, ctx) => {
    bot.onText(/\/respond \/(.+)\/ \[([.\s\S]*)\]/, (msg, match) => {
        ...
        if (!response && !isFile) {
            bot.sendMessage(chatId, "You didn't specify a response.").return;
        }
        ...
        try {
            ...
            if (isFile && response) {
                bot.sendMessage(
                    chatId,
                    "You can't specify a response if you send a file."
                );
                return;
            }
            ...
            bot.sendMessage(
                chatId,
                "You didn't specify a proper regex. It could not be parsed."
            );
            return;
        }
    });
    ...
    ['Video', 'Audio'].forEach(type => {
        bot.onText(
            new RegExp(`\\/respond \\/(.+)\\/ \\$${type.toLowerCase()}`),
            (msg, match) => {
                const chatId = msg.chat.id;
                const regex = match[1];
                ...
                if (!isFile) {
                    bot.sendMessage(
                        chatId,
                        'You have to send a file which contains a caption.'
                    );
                    return;
                }
                ...
                if (!isType) {
                    bot.sendMessage(
                        chatId,
                        `You have to send a valid ${lowerCaseType} file.`
                    );
                    return;
                }
    ...
    bot.onText(/\/respond \/(.+)\/ \$photo/, (msg, match) => {
        ...
        const fileId = msg.photo.sort((a, b) => b.file_size - a.file_size)[0]
            .file_id;
    ...
    bot.onText(/\/respond \/(.+)\/ \$voice/, (msg, match) => {
        ...
        if (!fileId) {
            bot.sendMessage(
                chatId,
                'You have to send a voice recording before calling this command.'
            );
            return;
        }
